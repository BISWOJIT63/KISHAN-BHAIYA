...
const documentOptions = {
  ...
  farmer: [
    ["PHOTO_ID", "Configured photo ID"],
    ["FARMING_EVIDENCE", "Optional farming evidence"],
  ],
  fpo_manager: [
...
const statusContent = {
  PENDING_ADMIN_APPROVAL: {
    ...
    description:
      "KISHAN BHAIYA verification staff will review your profile and submitted documents. Operational tools remain locked until approval.",
  ...
  CHANGES_REQUESTED: {
    ...
    description:
      "Review the administrator note, update your details or documents, then resubmit your application.",
  ...
  REJECTED: {
    ...
    description:
      "The decision and reason are preserved below. Contact support if you believe another review is required.",
  ...
  SUSPENDED: {
    ...
    description:
      "Your historical records are preserved, but operational access is disabled until an administrator reactivates the account.",
    tone: "border-red-200 bg-red-50 text-red-900",
    icon: AlertTriangle,
  },
  ACTIVE: {
...
export function VerificationCenterPage() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const result = await getData(api.get("/auth/verification"));
  ...
  const accountStatus =
    user?.accountStatus ||
    (user?.verified ? "ACTIVE" : "PENDING_ADMIN_APPROVAL");
  const content =
    statusContent[accountStatus] || statusContent.PENDING_ADMIN_APPROVAL;
  ...
  if (error)
    return (
      <div className="container-page py-16">
        <EmptyState
          title="Verification center unavailable"
    ...
    <div className="min-h-screen bg-cream">
      ...
      <main className="container-page max-w-6xl py-10">
        <PageHeader
          ...
          description="Review your application, securely provide configured evidence and track every decision."
        />
        <div className="grid items-start gap-6 lg:grid-cols-[320px_1fr]">
          <aside className="card p-6 text-center">
            ...
            <Link to="/profile" className="btn-secondary mt-6 w-full">
          ...
          <div className="space-y-6">
            <section className={`rounded-3xl border p-6 ${content.tone}`}>
              ...
              {data.profile.rejectionReasonCode && (
                <p className="mt-3 text-xs font-bold">
                  Reason:{" "}
                  {data.profile.rejectionReasonCode.replaceAll("_", " ")}
                </p>
              )}
            </section>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-4">
                {[
                  ...
                  <div key={label} className="rounded-2xl bg-cream p-4">
                    <span
                      className={`grid h-8 w-8 place-items-center rounded-full ${complete ? "bg-forest-700 text-white" : "bg-gray-200 text-gray-500"}`}
                    >
                      {complete ? (
            ...
            {accountStatus !== "ACTIVE" &&
              !["REJECTED", "SUSPENDED"].includes(accountStatus) && (
                <section className="card p-6 sm:p-8">
                  <div className="flex items-start gap-3">
                    <FileCheck2 className="mt-1 h-5 w-5 text-forest-700" />
                    <div>
                      <h2 className="font-display text-xl font-bold">
                        Verification documents
                      </h2>
                      <p className="mt-1 text-sm text-gray-500">
                        Files are stored privately and are never exposed through
                        public seller or traceability APIs.
                      </p>
                    </div>
                  </div>
                  <div className="mt-6 grid gap-4 sm:grid-cols-[1fr_1fr_auto]">
                    ...
                    <label>
                      <span className="label">PDF or image</span>
                      <input
                    ...
                    <button
                      ...
                      disabled={!document || uploadDocument.isPending}
                  ...
                  <div className="mt-6 space-y-3">
                    {data.documents.length ? (
                      ...
                      <p className="rounded-2xl bg-cream p-4 text-sm text-gray-500">
            ...
            {accountStatus === "ACTIVE" && (
              <Link to={workspaceForRole(user?.role)} className="btn-primary">
...
export function VerificationAdminPage() {
  ...
  const query = useQuery({
    queryKey: ["admin-verifications", status, role],
  ...
  const selected =
    applications.find((item) => item._id === selectedId) || applications[0];
  const review = useMutation({
    mutationFn: (action) => {
      if (!selected) throw new Error('Select an application first');
      if (['REQUEST_CHANGES', 'REJECT', 'SUSPEND'].includes(action) && !note.trim()) throw new Error('Enter a review note before continuing');
      return getData(
        api.patch(`/admin/verifications/${selected._id}/review`, {
          action,
    ...
    onSuccess: (result) => {
      toast.success(
        `Application ${result.overallStatus.toLowerCase().replaceAll("_", " ")}`,
      );
      setNote("");
      queryClient.setQueriesData({ queryKey: ['admin-verifications'] }, old => old?.map(item => item._id === result._id ? { ...item, ...result } : item));
      queryClient.invalidateQueries({ queryKey: ["admin-verifications"] });
      queryClient.invalidateQueries({ queryKey: ['bootstrap'] });
    },
    onError: (reviewError) => toast.error(reviewError.response ? apiError(reviewError) : reviewError.message),
  });
  return (
    <div className="container-page py-10">
      <PageHeader
        eyebrow="KISHAN BHAIYA Admin"
        title="Verification queue"
        description="Review applicants, document metadata and prior decisions. Every high-impact action creates an immutable audit record."
        actions={
      ...
      {query.isLoading ? (
        ...
        <EmptyState
          ...
          description={apiError(query.error)}
        />
      ) : (
        <div className="grid items-start gap-6 lg:grid-cols-[360px_1fr]">
          <aside className="card max-h-[65dvh] overflow-y-auto overscroll-contain p-4 lg:sticky lg:top-0">
            ...
            {applications.length ? (
              applications.map((application) => (
                <button
                  key={application._id}
                  className={`mt-2 w-full rounded-2xl border p-4 text-left ${selected?._id === application._id ? "border-forest-500 bg-forest-50" : "border-gray-200"}`}
                  onClick={() => {
              ...
              <EmptyState title="No matching applications" />
            )}
          </aside>
          <section>
            {selected ? (
              <div className="card overflow-hidden">
                <header className="border-b p-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <UserCheck className="h-8 w-8 text-forest-700" />
                  </div>
                </header>
                <div className="grid gap-7 p-6 xl:grid-cols-[1fr_320px]">
                  <div>
                    ...
                    <dl className="mt-4 grid gap-3 sm:grid-cols-2">
                      {[
                        ["Role", selected.role.replaceAll("_", " ")],
                        [
                          "Organization",
                          selected.applicant?.organization || "Not provided",
                        ],
                        [
                          "Location",
                          selected.applicant?.location || "Not provided",
                        ],
                        ["Resubmissions", selected.resubmissionCount || 0],
                      ].map(([label, value]) => (
                        <div key={label} className="rounded-2xl bg-cream p-4">
                        ...
                        selected.documents.map((item) => (
                          ...
                          >
                            <div>
                              ...
                              <p className="mt-1 text-xs text-gray-500">
                                {item.mimeType} · {Math.ceil(item.size / 1024)}{" "}
                                KB · private file
                              </p>
                            </div>
                            <StatusBadge status={item.status} />
                  ...
                  <aside>
                    ...
                    <label className="mt-4 block">
                      ...
                      >
                        <option value="INCOMPLETE_OR_UNVERIFIABLE">
                          Incomplete or unverifiable
                        </option>
                        <option value="DOCUMENT_MISMATCH">
                          Document mismatch
                        </option>
                        <option value="DUPLICATE_APPLICATION">
                          Duplicate application
                        </option>
                        <option value="OUTSIDE_POLICY">
                          Outside platform policy
                        </option>
                      </select>
                    </label>
                    <div className="mt-5 grid gap-2">
                      {selected.overallStatus !== "APPROVED" && (
                        <button
                      ...
                      {selected.overallStatus === "SUSPENDED" ? (
                        <button
                          className="btn-secondary"
                          disabled={review.isPending}
                    ...
                    <p className="mt-5 rounded-2xl bg-forest-50 p-4 text-xs leading-5 text-forest-800">
                      Private document file keys are intentionally omitted from
                      this API response. A future secure reviewer download must
                      use a short-lived authorized endpoint.
                    </p>
                  </aside>
                </div>
              </div>
