 ...
 */
export function IndicLanguageIcon({ className = "h-5 w-5" }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
...
export default function LanguageSwitcher({
  ...
  if (variant === "segmented") {
    return (
      <div
        ...
        className={cx(
          "grid grid-cols-4 gap-1 rounded-2xl bg-forest-50 p-1",
          className,
        )}
      >
        {languages.map((entry) => {
          const active = current === entry.code;
          return (
            <button
              key={entry.code}
              type="button"
              ...
              aria-pressed={active}
              className={cx(
                "flex min-h-11 flex-1 items-center justify-center rounded-xl px-2 text-sm font-bold transition",
                active
                  ? "bg-[#1d5f41] text-white shadow-sm"
  ...
  if (variant === "header") {
    return (
      ...
      >
        <span className="pointer-events-none absolute left-2.5 grid place-items-center">
          <IndicLanguageIcon className="h-5 w-5 text-[#1d5f41]" />
        </span>
        <select
          className="h-11 cursor-pointer appearance-none rounded-xl border border-gray-300 bg-white pl-9 pr-3 text-xs sm:text-sm font-bold text-gray-800 hover:bg-gray-50 focus:border-[#1d5f41] focus:outline-none focus:ring-2 focus:ring-[#1d5f41]/20 shadow-sm"
          value={current}
  ...
  return (
    ...
    >
      <IndicLanguageIcon className="h-5 w-5 shrink-0 text-[#1d5f41]" />
      {!compact && <span>{t("language.label")}</span>}
      <select
        className={cx("input h-11", compact && "w-[110px] px-2")}
        value={current}
        onChange={(event) => changeLanguage(event.target.value)}
        aria-label={t("language.label")}
      >
