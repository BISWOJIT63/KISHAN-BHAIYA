...
if (
  ...
  !/^kishan-bhaiya(?:-|$)/i.test(databaseName)
) {
  console.error(
    `Refusing to replace collections in non-demo database "${databaseName}". Use a database name beginning with kishan-bhaiya outside production.`,
  );
  process.exit(1);
}
...
console.log(
  `Kishan Bhaiya seed complete: ${data.products.length} products, ${data.lots.length} lots, ${data.requirements.length} requirements.`,
);
